...
class UserDash extends Dbh{
    ...
    protected function disableWaterMeter($meterid){
        $stmt = $this->connect()->prepare('UPDATE meters SET status_wat_user = 0 WHERE meterID = ?;');
        if(!$stmt->execute(array($meterid))){
            $stmt = NULL;
            exit();
        }
    ...
    protected function enableWaterMeter($meterid){
        $stmt = $this->connect()->prepare('UPDATE meters SET status_wat_user = 1 WHERE meterID = ?;');
        if(!$stmt->execute(array($meterid))){
            $stmt = NULL;
            exit();
        }
    ...
    protected function disableElecMeter($meterid){
        $stmt = $this->connect()->prepare('UPDATE meters SET status_elec_user = 0 WHERE meterID = ?;');
        if(!$stmt->execute(array($meterid))){
            $stmt = NULL;
            exit();
        }
    ...
    protected function enableElecMeter($meterid){
        $stmt = $this->connect()->prepare('UPDATE meters SET status_elec_user = 1 WHERE meterID = ?;');
        if(!$stmt->execute(array($meterid))){
            $stmt = NULL;
            exit();
        }
    ...
    protected function recharge($meterid,$elecVoucher,$waterVoucher){
        ...
        if($stmt->rowCount()==0){
            ...
            header("Location:../views/user-dash.php?success_elec");
            exit();
        }
        ...
        if($stmt->rowCount()==0){
            ...
            if(!$stmt->execute(array($newVoucher,$amountWater))){
             ...
             //header("Location:../views/user-dash.php?success");
            //exit();
            }
            header("Location:../views/user-dash.php?success_wat");
           exit();
            
        }
         header("Location:../views/user-dash.php?Invalid_voucher");
           exit();
           // header("Location:../views/user-dash.php?successful"); this
        //exit();
        ///////////////////////////////////////////////////////////////////
